...
{
    //-----------------------------------
    pthread_mutex_init(&m_writers, NULL);
    //-----------------------------------
...
int db_add(DB* self, Variant* key, Variant* value,    int i, char* writting_key, pthread_t t_id, int wichThread, int thread)
{
    ...
    int return_value = memtable_add(self->memtable, key, value,     i,  writting_key,  t_id,  wichThread, thread);     

    return return_value;
}

int db_get(DB* self, Variant* key, Variant* value,    int i, char* reading_key, pthread_t t_id, int wichThread, int thread)
{
    if (memtable_get(self->memtable->list, key, value,     i,  reading_key,  t_id,  wichThread, thread) == 1)
        return 1;

    return sst_get(self->sst, key, value,     i,  reading_key,  t_id,  wichThread, thread);
}

int db_remove(DB* self, Variant* key,    int i, char* reading_key, pthread_t t_id, int wichThread, int thread)
{
    return memtable_remove(self->memtable, key,     i,  reading_key,  t_id,  wichThread, thread);
}
