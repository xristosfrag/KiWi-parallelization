...
int sst_get(SST* self, Variant* key, Variant* value,    int i, char* reading_key, pthread_t t_id, int wichThread, int thread)
{
#ifdef BACKGROUND_MERGE
    int ret = 0;
    ...
    if (self->immutable)
    {
        ...
        ret = memtable_get(self->immutable_list, key, value,     i,  reading_key,  t_id,  wichThread, thread);
    }
    pthread_mutex_unlock(&self->cv_lock);
    ...
    if(thread == 1){
    	fprintf(stderr, "								Enabled writers = %d\n", writers);
    	fprintf(stderr, "IN SST:    %d searching %s, by thread: %ld -----started %dst\n", i, reading_key, t_id, wichThread);
    }
